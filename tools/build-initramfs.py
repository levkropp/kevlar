...
def compile_all_local():
    ...
    musl_bins = [
        ...
        ("testing/strace_target.c",      "strace-target",    []),
        ("testing/auxv_dump.c",          "auxv-dump",        []),
...
def build_k2_ko_arm64(cc):
    """Build the K2 demo `.ko` exercising kmalloc/wait_queue/
    completion/work_struct/msleep.  Same flags as hello.ko plus
    `-I testing/include` for kevlar_kabi_k2.h.
    """
    out = CACHE / "local-bin-arm64" / "k2.ko"
    src = ROOT / "testing" / "k2-module.c"
    ...
    try:
        ...
        log("WARN", f"k2.ko: build error: {e}")
        return None
    if r.returncode != 0:
        log("WARN", f"k2.ko: build failed: {(r.stderr or '').strip()[:300]}")
        return None
    log("CC", f"k2.ko built ({out.stat().st_size} bytes)")
    return out


def build_k3_ko_arm64(cc):
    """Build the K3 demo `.ko` exercising the device-model spine
    (struct device, struct platform_driver, etc.).  Same flags as
    K1/K2 plus `-I testing/include`.
    """
    out = CACHE / "local-bin-arm64" / "k3.ko"
    src = ROOT / "testing" / "k3-module.c"
    inc = ROOT / "testing" / "include"
    if not src.exists():
        return None
    out.parent.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        log("WARN", f"k3.ko: build error: {e}")
        return None
    if r.returncode != 0:
        log("WARN", f"k3.ko: build failed: {(r.stderr or '').strip()[:300]}")
        return None
    log("CC", f"k3.ko built ({out.stat().st_size} bytes)")
    return out


def build_k4_ko_arm64(cc):
    """Build the K4 demo `.ko` exercising file_operations + char-device."""
    out = CACHE / "local-bin-arm64" / "k4.ko"
    src = ROOT / "testing" / "k4-module.c"
    inc = ROOT / "testing" / "include"
    if not src.exists():
        return None
    out.parent.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        log("WARN", f"k4.ko: build error: {e}")
        return None
    if r.returncode != 0:
        log("WARN", f"k4.ko: build failed: {(r.stderr or '').strip()[:300]}")
        return None
    log("CC", f"k4.ko built ({out.stat().st_size} bytes)")
    return out
...
def assemble_rootfs_arm64(arm64_bins, local_arm64_bins=None, hello_ko=None, k2_ko=None, k3_ko=None, k4_ko=None, k5_ko=None, k6_ko=None, k7_ko=None, k8_ko=None):
    ...
    # ── kABI K2 demo module ──
    if k2_ko and k2_ko.is_file():
        modules_dir = ROOTFS / "lib" / "modules"
        modules_dir.mkdir(parents=True, exist_ok=True)
        dest = modules_dir / "k2.ko"
        if dest.exists():
            dest.unlink()
        shutil.copy2(k2_ko, dest)
        log("MOD", f"installed /lib/modules/k2.ko ({k2_ko.stat().st_size} bytes)")

    # ── kABI K3 demo module ──
    if k3_ko and k3_ko.is_file():
        modules_dir = ROOTFS / "lib" / "modules"
        modules_dir.mkdir(parents=True, exist_ok=True)
        dest = modules_dir / "k3.ko"
        if dest.exists():
            dest.unlink()
        shutil.copy2(k3_ko, dest)
        log("MOD", f"installed /lib/modules/k3.ko ({k3_ko.stat().st_size} bytes)")

    # ── kABI K4 demo module ──
    if k4_ko and k4_ko.is_file():
        modules_dir = ROOTFS / "lib" / "modules"
        modules_dir.mkdir(parents=True, exist_ok=True)
        dest = modules_dir / "k4.ko"
        if dest.exists():
            dest.unlink()
    ...
    # 21 undef symbols, mostly kmem_cache_* + rb_* + list debug.
    # First DRM-stack module.
    ...
    # ── kABI K14 target: drm_exec.ko (DRM transactional buffer
    # reservation helper).  Pure library module (no init_module).
    # 11 undef symbols across ww_mutex / dma_resv / drm_gem /
    # refcount / kvmalloc renames.
    drm_exec_src = ROOT / "build" / "linux-modules" / "lib" / "modules" / \
        "7.0.0-14-generic" / "kernel" / "drivers" / "gpu" / "drm" / "drm_exec.ko"
    if drm_exec_src.exists():
        modules_dir = ROOTFS / "lib" / "modules"
        modules_dir.mkdir(parents=True, exist_ok=True)
        dest = modules_dir / "drm_exec.ko"
        if dest.exists():
            dest.unlink()
    ...
    # ── kABI K16 target: drm_dma_helper.ko (DMA-coherent GEM
    # buffer helper).  79 undefs (32 net new) across DMA API,
    # DRM GEM/prime/atomic, drm_client/format extensions, and
    # mm helpers.  Pure library module.
    drm_dma_helper_src = ROOT / "build" / "linux-modules" / "lib" / "modules" / \
        "7.0.0-14-generic" / "kernel" / "drivers" / "gpu" / "drm" / "drm_dma_helper.ko"
    if drm_dma_helper_src.exists():
        modules_dir = ROOTFS / "lib" / "modules"
        modules_dir.mkdir(parents=True, exist_ok=True)
        dest = modules_dir / "drm_dma_helper.ko"
        if dest.exists():
            dest.unlink()
