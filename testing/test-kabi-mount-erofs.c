...
// filesystem.  Runs as PID 1 (init=/bin/test-kabi-mount-erofs).
// Exercises the full syscall path: mount(2) → opendir/readdir →
// open/read against /mnt/erofs/{hello.txt,info.txt}.
//
// Requires the kernel cmdline to load erofs.ko + enable fill_super:
...
int main(void) {
    write(1, "TEST_START kabi_mount_erofs\n", 28);

    /* 1. Mount erofs at /mnt/erofs.  /mnt and /mnt/erofs are
     *    pre-created by tools/build-initramfs.py (initramfs is RO at
     *    runtime, so we can't mkdir at boot). */
    int r = mount("/lib/test.erofs", "/mnt/erofs", "erofs",
                  MS_RDONLY, NULL);
    int em = errno;
    check("mount_erofs", r == 0);
    if (r != 0) {
        char line[64];
        int len = snprintf(line, sizeof(line),
                           "mount r=%d errno=%d\n", r, em);
        write(1, line, len);
        write(1, "TEST_FAIL kabi_mount_erofs\n", 27);
        write(1, "TEST_END\n", 9);
        return 1;
    }

    /* 2. opendir + readdir, collect names. */
    DIR *d = opendir("/mnt/erofs");
    check("opendir_mnt_erofs", d != NULL);
    ...
    if (fd >= 0) {
        ...
        const char *expect = "hello from kABI-mounted erofs!\n";
    ...
    fd = open("/mnt/erofs/hello.txt", O_RDONLY);
    if (fd >= 0) {
        lseek(fd, 6, SEEK_SET);
        char buf[16] = {0};
        int n = read(fd, buf, sizeof(buf) - 1);
    ...
    write(1, line, len);
    if (fail_count == 0) write(1, "TEST_PASS kabi_mount_erofs\n", 27);
    else                 write(1, "TEST_FAIL kabi_mount_erofs\n", 27);
    write(1, "TEST_END\n", 9);
