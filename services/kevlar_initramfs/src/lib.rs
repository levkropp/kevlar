...
//! Initramfs (cpio newc) parser for Kevlar.
//!
//! This is a Ring 2 service crate — it depends only on `kevlar_vfs` traits
//! and `kevlar_utils`.  It contains no `unsafe` code.
//!
//! The `init()` function that calls `include_bytes!` remains in the kernel
//! crate because it depends on the `INITRAMFS_PATH` build-time env var.
#![no_std]
#![forbid(unsafe_code)]
...
impl Directory for InitramFsDir {
    ...
    fn lookup(&self, name: &str) -> Result<INode> {
        ...
        Ok(match initramfs_inode {
            ...
            InitramFsINode::Directory(dir) => {
                INode::Directory(dir.clone() as Arc<dyn Directory>)
            }
...
impl SymlinkTrait for InitramFsSymlink {
    fn stat(&self) -> Result<Stat> {
        Ok(self.stat)
    }
...
impl InitramFs {
    pub fn new(fs_image: &'static [u8]) -> InitramFs {
        ...
        loop {
            ...
            log::trace!("initramfs: \"{}\" ({})", path, ByteSize::new(filesize));
